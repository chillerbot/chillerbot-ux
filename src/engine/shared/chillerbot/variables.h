...
MACRO_CONFIG_INT(ClSaveMapInfo, cl_save_map_info, 1, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Save map info ( off for tml compatibility on for everything else)")
...
MACRO_CONFIG_INT(ClMmoTee, cl_mmotee, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Hide MMOTEE chat messages to avoid chat spam")
...
MACRO_CONFIG_INT(ClDbgIntersect, cl_dbg_intersect, 0, 0, 1, CFGFLAG_CLIENT, "Show graphical output for CCollision::IntersectLine")
...
MACRO_CONFIG_INT(ClChillpw, cl_chillpw, 1, 0, 1, CFGFLAG_CLIENT, "turn password manager auto login on and off")
MACRO_CONFIG_INT(BrFilterNoIpv6, br_filter_no_ipv6, 0, 0, 1, CFGFLAG_SAVE | CFGFLAG_CLIENT, "Filter ipv4 only servers")
